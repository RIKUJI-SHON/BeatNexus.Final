        ...
        <>
          ...
          <div className="text-center mb-4">
            <div className="text-xs text-gray-400 mb-1">
              {t('monthlyLimit.monthlySubmissions')}
            </div>
            <div className="text-3xl font-bold text-cyan-400 mb-2">
              {monthlyData.used_count}
            </div>
            <div className="text-sm text-gray-300">
              {monthlyData.remaining === Infinity 
                ? t('monthlyLimit.unlimitedSubmissions') 
                : t('monthlyLimit.remainingSubmissions', { count: monthlyData.remaining })
              }
            </div>
          </div>
          ...
          <div className="p-3 bg-slate-700/30 rounded-lg border border-slate-600/30">
            <div className="flex items-center justify-center gap-2 text-xs text-gray-300">
              ...
              <span>{t('monthlyLimit.nextReset', { date: monthlyData.reset_date })}</span>
            </div>
          </div>
          ...
          <div className="mt-3 pt-3 border-t border-slate-700/50">
            <p className="text-xs text-gray-400 text-center leading-relaxed">
              {t('monthlyLimit.testPeriodMessage')}
            </p>
          </div>
        </>
      )}
