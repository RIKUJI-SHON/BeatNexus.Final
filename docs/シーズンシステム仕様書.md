...
2. **【Phase 2】バトルランキングアーカイブ**（当該シーズン参加者のみ）
...
```sql
-- シーズン終了: 日本時間翌日00:00
jobid: 10, schedule: '0 15 * * *', command: 'SELECT public.end_current_season();'
...
- **日本時間（JST）**: UTC+9
- **サマータイム**: なし（年間固定）
- **実行間隔**: 19:00開始 → 翌日00:00終了（5時間サイクル）
...
### 6.2 アーカイブ条件
- 【変更】バトルランキング: 当該シーズンでの参加者のみ（archived_battlesに当該season_idでplayer1/2として1件以上存在）
- 投票者ランキング: season_vote_points >= 1（従来通り）
...
#### 2025-08-08
- シーズンランキングの対象を「当該シーズン参加者のみ」に変更
- end_current_seasonのアーカイブ条件を当該シーズン参加者に限定
