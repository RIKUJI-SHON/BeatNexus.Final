...
**最終更新**: 2025年9月22日  
**バージョン**: v8.1.1 (一言コメント追加・非破壊ラッパーRPC)  
**適用環境**: 開発環境・本番環境
...
- NEW: **投稿「一言コメント」**（任意・最大140文字）をサポート。非破壊のラッパーRPCで既存フローを維持。
...
- 入力: 任意テキスト。空白のみは無効化され、DB上はNULLとして扱われます。
- 最大長: 140文字（DBチェック制約およびRPCバリデーションで保証）。
- 保存先: `public.submissions.one_line_comment`（NULL許容）。
- 呼び出し: コメントがある場合は `create_submission_with_comment` を利用。コメント未入力時は従来の `create_submission_with_cooldown_check` を利用。
- エラーハンドリング: 140文字超過時は投稿を行わず、以下のエラーを返却します。
