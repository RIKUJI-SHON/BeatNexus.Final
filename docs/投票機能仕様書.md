...
**最終更新**: 2025年9月3日  
**バージョン**: v7 (Score Sheet UI & Cancel Removal)  
**適用環境**: 開発環境・本番環境
...
3. [投票関数仕様](#投票関数仕様)
4. [ポイントシステム](#ポイントシステム)
5. [スコアシート（コメント投票モーダル）](#スコアシートコメント投票モーダル)
...
- **スコアシート（UI補助）**: コメント投票モーダル内のスイッチで有効化。スキル/音楽性/オリジナリティ（各100点満点）をA/Bそれぞれ入力し、合計を表示（ポイントや集計には影響しない）。
...
### 概要
- コメント投票モーダル内の「応援オプション（Super Tip）」の下に「スコアシートを使う」スイッチを設置。
- スイッチONで、以下の3セクションが表示されます（各100点満点）。
  - スキル（Skills）
  - 音楽性（Musicality）
  - オリジナリティ（Originality）
- 各セクションで Player A/B のスコアを入力すると、A/Bそれぞれの合計（最大300点）が表示されます。

### 注意事項
- 現時点ではUI補助機能であり、入力値はサーバーに送信されず、ポイント・ランキング計算に影響しません。
...
| 2 | 非アクティブ | コメント付き投票 | vote_count: +3, season_vote_points: +0 | ✅ 正常 | PASS |
| 3 | アクティブ化 | シーズン切り替え | 2025-S7をアクティブに | ✅ 正常 | PASS |
| 4 | アクティブ | 通常投票 | vote_count: +1, season_vote_points: +1 | ✅ 正常 | PASS |
| 5 | アクティブ | コメント付き投票 | vote_count: +3, season_vote_points: +3 | ✅ 正常 | PASS |
...
### v7 (2025-09-03) - Score Sheet UI & Cancel Removal
変更点:
1. コメント投票モーダルに「スコアシートを使う」スイッチと評価入力UI（スキル/音楽性/オリジナリティ、各100点、A/B合計表示）を追加。
2. 投票キャンセル機能を廃止（`cancel_vote` 関数の利用を停止、権限記載も削除）。
3. 投票ガイドボタン（Judge scorecard誘導）のUIを削除。
