...
- **バージョン**: 1.0
- **対象環境**: 本番環境・開発環境共通
- **最終更新**: 投票率タイブレーク・固定配点（fixed_points_v1）・i18n反映
...
#### 1.2 シーズンランキング
- **概要**: 現在シーズンのポイントランキング
- **評価指標**: シーズンポイント（固定配点: 勝+16 / 負+4 / 引+8）
...
### 4.3 シーズンポイント計算方式（fixed_points_v1）

- レーティング（Elo）は従来通り変更なし。
- シーズンポイントは固定配点（勝+16 / 負+4 / 引+8）で加算。
- シーズンポイントの下限は 1100（これ未満に低下しない）。
- 内部識別子: calculation_method = 'fixed_points_v1'。
