...
- **シーズン終了前非表示**: シーズン終了3日前からランキング非表示でワクワク感演出
...
### 8.1.1 シーズン終了前ランキング非表示

**目的**: シーズン終了後の順位発表にワクワク感を演出

**適用条件**: シーズン終了3日前から適用

**非表示対象**: **現在のアクティブシーズンランキングのみ**
- 通算ランキング（All Time）は表示継続
- 過去シーズンランキングは表示継続
- 現在シーズン（Current Season）選択時のみ非表示

**対象ページ**:
- `/ranking` - 現在シーズンランキング表示時のみ
- `/battles` - TabbedRankingコンポーネント（シーズンランキング表示）
...
**技術仕様**:
- 判定ロジック: `useSubmissionStatus`フックを活用
- 基本条件: `submissionStatus.canSubmit === false && submissionStatus.reason === 'ENDING_SOON'`
- 追加条件（RankingPage）:
  - プレイヤーランキング: `activeRankingType === 'current_season' && 現在シーズン選択中`
  - 投票者ランキング: `activeVoterRankingType === 'current_season' && 現在シーズン選択中`
- 表示制御: 現在シーズンランキング表示時のみ条件分岐で代替UI表示
...
### 2025-08-29
- **シーズン終了前ランキング非表示機能実装**: ワクワク感演出のためシーズン終了3日前から**現在のアクティブシーズンランキングのみ**を非表示化
  - 対象: 現在シーズンランキング表示時のみ（通算・過去シーズンは表示継続）
  - ページ: ランキングページ（/ranking）とバトルページ（/battles）のTabbedRanking
  - 判定ロジック: 既存の投稿制限と同じ`useSubmissionStatus`フックを活用
  - UI: トロフィーアイコンとカウントダウン付きの魅力的な代替メッセージ表示
  - 多言語対応: 日本語・英語で「順位発表まで、もう少し！」「Rankings Reveal Coming Soon!」
