...
**最終更新**: 2025年9月3日  
**バージョン**: v7.4 (MINI形式のレーティング/K係数・シーズンポイントをMAINの1/2に統一)  
**適用環境**: 開発環境・本番環境
...
- **バトル形式別Kファクター**: MAIN_BATTLE/MINI_BATTLE/THEME_CHALLENGEごとの調整（MINIはMAINの1/2）
- **シーズンポイント管理**: 連敗デケイ付き固定配点制。MINIはMAINの1/2（Win +8 / Draw +4 / Loss 2→1→0）
 - **シーズンポイント管理**: 連敗デケイ付き固定配点制。MINIはMAINの1/2（Win +16 / Draw +4 / Loss 2→1→0）
...
CASE battle_format
  WHEN 'MAIN_BATTLE' THEN RETURN 64;      -- メインバトル: 高変動
  WHEN 'MINI_BATTLE' THEN RETURN 32;      -- ミニバトル: MAINの1/2
...
- **固定配点 + 連敗デケイ（形式別）**:
  - MAIN_BATTLE: Win +32 / Draw +8 / Loss +4→+2→+0
  - MINI_BATTLE: Win +16 / Draw +4 / Loss +2→+1→+0（MAINの1/2に統一）
...
| 連続敗北数(直前) | 今回敗北時付与 | 説明 |
|------------------|---------------|------|
| 0 (初敗北)       | +4 (MAIN) / +2 (MINI) | デフォルト敗北ポイント |
| 1                | +2 (MAIN) / +1 (MINI) | 2連敗目で半減 |
| 2以上            | +0            | 3連敗目以降は付与なし |
...
MINIの場合の勝利/引き分け配点はそれぞれ +16 / +4 となります。
