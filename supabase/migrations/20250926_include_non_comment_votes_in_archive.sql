-- Migration: include non-comment votes in archived_battle_votes
-- Date: 2025-09-26
-- Description: Remove comment-only filter when copying votes during battle completion so that all votes (with or without comments) are archived.
...
BEGIN
  ...
  -- Copy ALL votes (comments optional) including score_sheet (removed comment-only filter)
  INSERT INTO public.archived_battle_votes (archived_battle_id, user_id, vote, comment, created_at, score_sheet)
  SELECT v_archived_battle_id, bv.user_id, bv.vote, NULLIF(bv.comment, '') AS comment, bv.created_at, bv.score_sheet
  ...
  -- Notifications (keep dev-env safe handling)
  BEGIN
  ...
  EXCEPTION WHEN undefined_table THEN
    -- notifications table not present (dev minimal schema) – ignore
    NULL;
  END;
