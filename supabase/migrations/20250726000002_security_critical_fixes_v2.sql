...
-- 匿名ユーザー用の制限付きアクセスポリシー（最小限の情報のみ）
-- usernameとavatar_urlのみ公開
...
-- ただし、匿名ユーザーが見える列を制限するためのセキュリティビューを作成
-- （ビューにはRLSポリシーを設定できないため、関数で制御）
