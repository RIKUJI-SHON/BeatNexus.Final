-- Apply SuperTip vote on payment success (service-side)
-- SECURITY DEFINER function to be called from Edge webhook.
...
BEGIN
  ...
  IF NOT FOUND THEN
    ...
    INSERT INTO public.battle_votes (
      ...
      GREATEST(p_super_tip_amount, 0), p_payment_intent_id, 'completed',
    ...
    v_is_new := true;
  ELSE
    -- Already voted: do not change counts; attach super tip linkage and details
...
IS 'Called by webhook on payment_intent.succeeded to materialize a vote from a SuperTip. Inserts battle_votes if not existing and increments counters.';
