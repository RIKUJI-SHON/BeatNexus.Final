/*
  # 🔐 セキュリティ緊急修正マイグレーション（Production Ready版）
  ...
  本番対応：既存データとの互換性を保ちながら段階的にセキュリティを強化
*/
...
-- 匿名投票データの削除（セキュリティ強化のため）
DELETE FROM public.battle_votes WHERE user_id IS NULL;
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE id NOT IN (
      SELECT DISTINCT ON (battle_id, user_id) id
      FROM public.battle_votes
      ORDER BY battle_id, user_id, created_at DESC
    );
  ...
  -- user_id_required制約を追加（データクリーンアップ後）
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
...
-- auth.usersテーブルのパスワード変更を監視するトリガー
-- （Supabaseの制限により、直接auth.usersにトリガーは設定できない場合があるので、コメントアウト）
-- CREATE TRIGGER log_password_change_trigger
--   AFTER UPDATE OF encrypted_password ON auth.users
--   FOR EACH ROW EXECUTE FUNCTION public.log_password_change();
...
BEGIN
  ...
  RAISE NOTICE '1. ✅ 既存データクリーンアップ完了';
  RAISE NOTICE '2. ✅ 匿名アクセス制限完了';
  RAISE NOTICE '3. ✅ SECURITY DEFINER関数強化完了';
  RAISE NOTICE '4. ✅ 投票システム不正防止完了';
  RAISE NOTICE '5. ✅ 監査ログシステム作成完了';
  RAISE NOTICE '6. ✅ セキュリティイベントログ機能追加完了';
  RAISE NOTICE '========================================';
  RAISE NOTICE '🔐 セキュリティレベルが大幅に向上しました';
  RAISE NOTICE '📊 次回: セキュリティ監視とアラートシステムの導入を推奨';
  RAISE NOTICE '========================================';
END $$;
