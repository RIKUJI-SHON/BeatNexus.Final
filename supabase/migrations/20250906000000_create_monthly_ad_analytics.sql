...
create unique index if not exists mv_ad_stats_monthly_unique_idx 
    on public.mv_ad_stats_monthly(month_start, simple_ad_id, placement_id, flight_id); -- 本番環境ではflight_id削除
...
-- 6. 自動リフレッシュ用のスケジューリング準備
-- 注意: この部分は pg_cron 拡張機能が必要です
-- 本番環境で月次自動リフレッシュを設定する場合:
-- SELECT cron.schedule('monthly-ad-stats', '0 1 1 * *', 'SELECT public.refresh_mv_ad_stats_monthly();');
...
-- 使用方法メモ:
-- 1. 月次データリフレッシュ: SELECT public.refresh_mv_ad_stats_monthly();
-- 2. 月次レポート生成: SELECT * FROM public.generate_monthly_ad_report('2025-09-01');
-- 3. パフォーマンス異常検知: SELECT * FROM public.detect_ad_performance_anomalies(3);
-- 4. トレンド分析: SELECT * FROM public.vw_monthly_performance_trends WHERE month_start >= '2025-01-01';
