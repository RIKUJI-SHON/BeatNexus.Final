-- 20250829154000_add_season_end_notifications.sql
-- 目的:
-- 1) notifications.type のチェック制約に 'season_end' を追加（既存種別も統合）
-- 2) public.end_current_season() に、シーズン終了時の一括通知作成（type='season_end', related_season_id=<終了シーズンID>）を追加

BEGIN;

-- 1) notifications.type の制約を再作成（既存の全種別 + season_end を包含）
...
-- 2) end_current_season() を更新して、シーズン終了後に season_end 通知を作成
--    ベースは直近の実装（20250815173000_apply_archive_tiebreaks.sql）に追記
...
BEGIN
  ...
  SELECT 
    ...
    NOW()
  FROM auth.users u
  WHERE u.id IN (SELECT id FROM public.profiles WHERE is_deleted = FALSE);
...
IS 'シーズン終了処理：アクティブバトル強制終了・ランキングアーカイブ・ポイントリセット・全ユーザーへseason_end通知作成';
