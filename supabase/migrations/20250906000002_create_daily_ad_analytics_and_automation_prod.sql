-- 日次広告分析システムと自動化ジョブ設定（本番環境用）
-- 作成日: 2025-09-06
-- 目的: 日次CTR監視、異常検知、自動レポーティング（本番環境）

begin;

-- 1. 日次集計ビュー（本番環境版 - flight_idなし）
...
begin
    ...
    with baseline_stats as (
        select 
            d.simple_ad_id,
            d.placement_id,
            avg(d.impressions) as avg_impressions,
            avg(d.ctr_percentage) as avg_ctr,
            stddev(d.impressions) as stddev_impressions,
            stddev(d.ctr_percentage) as stddev_ctr,
            count(*) as data_points
        from public.vw_ad_stats_daily d
        where d.event_date >= target_date - interval '1 day' * lookback_days
        and d.event_date < target_date
        and d.impressions >= 5
        group by d.simple_ad_id, d.placement_id
        having count(*) >= 3
    ),
    target_stats as (
        select 
            ...
            d.ctr_percentage
        from public.vw_ad_stats_daily d
        where d.event_date = target_date
        and d.impressions >= 5
    )
    -- インプレッション数異常検知
    select 
    ...
    from target_stats t
    join baseline_stats b on (t.simple_ad_id = b.simple_ad_id and t.placement_id = b.placement_id)
...
-- 本番環境固有の注意事項:
-- - flight_idカラムは存在しないため除外済み
-- - スキーマ構造は開発環境と一致するよう調整済み
-- - 自動化ジョブは本番環境のcronスケジューラーで実行
