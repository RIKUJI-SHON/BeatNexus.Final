-- 20250815173000_apply_archive_tiebreaks.sql
-- 目的: シーズン終了時のアーカイブ順位を season_points の同点時に
--       season_user_metrics のタイブレーク指標（weighted_vote_share, sum_margin_ratio, battles_played, last_battle_at, user_id）で決定する。
-- 影響範囲: public.end_current_season() の Phase 1（season_rankings 挿入部）のみ。その他の機能は維持。
...
IS 'シーズン終了処理：アクティブバトル強制終了・ランキングアーカイブ（タイブレーク込みのDENSE_RANK）・ポイントリセット';
