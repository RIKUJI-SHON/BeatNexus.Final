...
serve(async (req) => {
  ...
  try {
    ...
    // Supabase管理クライアント初期化（サービスロールキー使用）
    ...
    if (action === 'send_code') {
      ...
      console.log('📞 Normalized phone number for availability check');
      ...
      if (availabilityError) {
        ...
        return corsResponse({ error: 'system_error', message: 'システムエラーが発生しました' }, 500);
      }

      if (!availabilityResult?.available) {
        console.warn('⚠️ Phone number already registered');
        return corsResponse({ 
    ...
    if (action === 'verify_code') {
      ...
      if (authHeader && authHeader.startsWith('Bearer ')) {
        try {
          ...
          if (user && !authError) {
            console.log('👤 Recording phone verification for authenticated user');
            ...
            console.log('✅ Phone verification recorded successfully');
            return corsResponse({ 
              success: true, 
          ...
          console.warn('⚠️ Auth token processing failed (may be normal during sign-up):', e.message);
        }
      }
